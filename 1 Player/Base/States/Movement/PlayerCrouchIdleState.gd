class_name PlayerCrouchIdleState extends PlayerMovementState

func Enter():
	PLAYER.anim_player_audio.stop()
	#if PLAYER.current_player_state == PLAYER.Player_state_modes.AIR or PLAYER.current_player_state == PLAYER.Player_state_modes.CROUCHMOVE:
		#PLAYER.anim_player.play("crouch_skip")
		#if PLAYER.current_player_state != PLAYER.Player_state_modes.CROUCHMOVE:
			#PLAYER.anim_player_ui.play("Crouch_fade")
	#else:
		#PLAYER.anim_player.play("crouch_anim")
		#PLAYER.anim_player_ui.play("Crouch_fade")
	PLAYER.current_player_state = PLAYER.Player_state_modes.CROUCHIDLE
...
func Update(_delta: float):
	...
	if PLAYER.is_on_floor():
		...
		if Input.is_action_pressed("Crouch"):
			if PLAYER.velocity.length() > 0.0:
				Transition.emit(self,"PlayerCrouchMoveState")
